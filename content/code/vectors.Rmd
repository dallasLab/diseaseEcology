...
![](figs/mosqContact2.png){width="100%"}

<sub> Thongsripong et al. 2021 _Ann Entomol Soc Am_ </sub>
...
![](figs/mosqContact1.png){width="80%"}
...
<sub> $R_0$ estimates for COVID; Dhungel et al. 2022 _Int J Environ Res Public Health_</sub> 
...
```{r, echo=FALSE}
par(mar=c(4,4,0,0))
plot(y=R0(Nv=10, Nh=10, a=0.5, bH=0.5, bV=0.5, p=0.95, r=seq(0.2, 1, length.out=100)), 
  x=seq(0.2, 1, length.out=100), las=1,
  xlab='Host recovery rate', ylab=expression(R[0]), type='b', pch=16, col='dodgerblue')
abline(h=1, lwd=2, col='grey')
...
```{r, echo=FALSE}
par(mar=c(4,4,0,0))
plot(y=R0(Nv=10, Nh=10, a=0.5, bH=0.5, bV=0.5, p=seq(0.2, 1, length.out=100), r=0.7), 
  x=seq(0.2, 1, length.out=100), las=1,
  xlab='Mosquito survival probability', ylab=expression(R[0]), type='b', pch=16, col='dodgerblue')
abline(h=1, lwd=2, col='grey')
...
colz <- viridis::viridis(5)
par(mar=c(4,4,0.25,0.25))
plot(y=R0(Nv=10, Nh=10, a=0.5, bH=0.5, bV=0.5, p=0.95, r=seq(0.2, 1, length.out=100)), 
  x=seq(0.2, 1, length.out=100), las=1, ylim=c(0,4),
  xlab='Host recovery rate', ylab=expression(R[0]), type='b', pch=16, col=colz[1])
lines(y=R0(Nv=10, Nh=10, a=0.5, bH=0.5, bV=0.5, p=0.94, r=seq(0.2, 1, length.out=100)), 
  x=seq(0.2, 1, length.out=100),
  type='b', pch=16, col=colz[2])
lines(y=R0(Nv=10, Nh=10, a=0.5, bH=0.5, bV=0.5, p=0.93, r=seq(0.2, 1, length.out=100)), 
  x=seq(0.2, 1, length.out=100),
  type='b', pch=16, col=colz[3])
lines(y=R0(Nv=10, Nh=10, a=0.5, bH=0.5, bV=0.5, p=0.92, r=seq(0.2, 1, length.out=100)), 
  x=seq(0.2, 1, length.out=100),
  type='b', pch=16, col=colz[4])
lines(y=R0(Nv=10, Nh=10, a=0.5, bH=0.5, bV=0.5, p=0.91, r=seq(0.2, 1, length.out=100)), 
  x=seq(0.2, 1, length.out=100),
  type='b', pch=16, col=colz[5])

abline(h=1, lwd=2, col='grey')

legend('topright', c('p=0.95', 'p=0.94', 'p=0.93', 'p=0.92', 'p=0.91'), 
  col=colz, pch=16, bty='n')
...
This points to the importance of the <font color='green'> reservoir host community </font>,  <font color='green'> environmental control of pathogen presence </font>, and/or <font color='green'> mitigation efforts </font> 

<sub> But probably not mitigation efforts. Does Columbia spray for mosquitos? No. Other counties do. </sub>
...
<br>
<br>
<h3> How would we go about incorporating vector biting preferences and the underlying host community in our estimate of $R_0$? </h3>
...
+ Do we need to model those parameters directly as functions of environmental variables?   
  + Mordecai et al. 2019 _Ecology Letters_ argues this
